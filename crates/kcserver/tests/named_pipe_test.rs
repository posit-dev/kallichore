...
mod windows_named_pipe_tests {
    ...
    async fn test_named_pipe_server_startup() {
        ...
        match connect_to_named_pipe(server.pipe_name()).await {
            Ok(_) => {
                println!("Successfully connected to named pipe: {}", server.pipe_name());
            }
            Err(e) => {
        ...
        match send_http_request(
            ...
            Ok(response) => {
                ...
                if response.status_code == 200 {
                    ...
                    println!("✓ HTTP over named pipe responded with status: {}", response.status_code);
                }
            }
            Err(e) => {
                println!("Warning: Failed to send HTTP request over named pipe: {}", e);
    ...
    async fn test_named_pipe_channels_upgrade() {
        ...
        if let Ok(create_response) = send_http_request(
            ...
            println!("Session creation response: HTTP {}", create_response.status_code);
            ...
            if create_response.status_code == 200 {
                ...
                match send_http_request(
                    ...
                    Ok(upgrade_response) => {
                        println!("Channels upgrade response: HTTP {}", upgrade_response.status_code);
                        println!("Response body: {}", upgrade_response.body);
                        
                        if upgrade_response.status_code == 200 {
                            println!("✓ Channels upgrade endpoint is working over named pipes");
                        } else {
                            println!("✓ Channels upgrade endpoint responded (status: {})", upgrade_response.status_code);
                        }
                    }
                    Err(e) => {
    ...
    async fn test_named_pipe_session_creation_and_listing() {
        ...
        // Wait for the server to be ready
        tokio::time::sleep(Duration::from_secs(3)).await;        let session_id = format!("test-session-{}", uuid::Uuid::new_v4());
        let session_request = json!({
            "session_id": session_id,
        ...
        let create_response = send_http_request(
            ...
            "/sessions",
            Some(&session_request.to_string()),
        ).await.expect("Failed to send session creation request");

        println!("Session creation response: HTTP {}", create_response.status_code);
        println!("Response body: {}", create_response.body);

        // Verify session was created successfully
        assert_eq!(create_response.status_code, 200, "Session creation should succeed");
        
        // Parse the response to get the session ID
        let response_json: serde_json::Value = serde_json::from_str(&create_response.body)
            .expect("Response should be valid JSON");
        
        let returned_session_id = response_json["session_id"].as_str()
        ...
        let list_response = send_http_request(
            server.pipe_name(),
            "GET",
            "/sessions",
            None,
        ).await.expect("Failed to send session list request");

        println!("Session list response: HTTP {}", list_response.status_code);
        println!("Response body: {}", list_response.body);

        // Verify we got a successful response
        assert_eq!(list_response.status_code, 200, "Session listing should succeed");
        ...
        let total = sessions_json["total"].as_i64()
        ...
        let found_session = sessions.iter().find(|session| {
            session["session_id"].as_str() == Some(&session_id)
        });
        ...
        assert_eq!(found_session["display_name"].as_str().unwrap(), "Python Test Session");
        assert_eq!(found_session["language"].as_str().unwrap(), "python");
        
        println!("✓ Session '{}' successfully found in session list", session_id);
