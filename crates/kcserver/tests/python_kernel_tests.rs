...
#[cfg(target_os = "windows")] // Windows-specific test
#[tokio::test]
async fn test_startup_environment_command_mode_windows() {
    let test_result = tokio::time::timeout(Duration::from_secs(60), async {
        let python_cmd = if let Some(cmd) = get_python_executable().await {
            cmd
        } else {
        ...
        let session_id = format!("test-command-mode-windows-{}", Uuid::new_v4());

        // Create a session with StartupEnvironment::Command that sets an environment variable using PowerShell
        let new_session = NewSession {
            session_id: session_id.clone(),
            display_name: "Test Command Mode Session (Windows)".to_string(),
            language: "python".to_string(),
            username: "testuser".to_string(),
            input_prompt: "In [{}]: ".to_string(),
            continuation_prompt: "   ...: ".to_string(),
            ...
            protocol_version: Some("5.3".to_string()),
            startup_environment: kallichore_api::models::StartupEnvironment::Command,
            startup_environment_arg: Some("$env:TEST_KALLI_VAR='from_command_windows'".to_string()),
        };

        println!("Creating session with StartupEnvironment::Command on Windows");

        // Create the session
        let _created_session_id = create_session_with_client(&client, new_session).await;

        // Start the session
        println!("Starting kernel session with command mode on Windows...");
        let start_response = client
            .start_session(session_id.clone())
            .await
            .expect("Failed to start session");
...
"#;
        ...
        // Verify the environment variable was set correctly
        assert!(
            all_output.contains("TEST_KALLI_VAR=from_command_windows"),
            "Expected to find 'TEST_KALLI_VAR=from_command_windows' in output, got: {}",
            all_output
        );

        println!("✓ Command mode test passed! Environment variable was set correctly on Windows");
    ...
    match test_result {
        Ok(_) => {
            println!("Startup environment command mode test (Windows) completed successfully");
        }
        Err(_) => {
            panic!("Startup environment command mode test (Windows) timed out after 60 seconds");
        }
    }
}
...
#[cfg(target_os = "windows")] // Windows-specific test
#[tokio::test]
async fn test_startup_environment_script_mode_windows() {
    let test_result = tokio::time::timeout(Duration::from_secs(60), async {
        let python_cmd = if let Some(cmd) = get_python_executable().await {
            cmd
        } else {
        ...
        // Create a temporary startup script for Windows (PowerShell)
        let temp_dir = tempfile::tempdir().unwrap();
        let script_path = temp_dir.path().join("startup_script.ps1");
        std::fs::write(
            &script_path,
            "$env:TEST_KALLI_VAR='from_script_windows'\n",
        )
        .unwrap();
        ...
        let session_id = format!("test-script-mode-windows-{}", Uuid::new_v4());

        // Create a session with StartupEnvironment::Script that sets an environment variable
        let new_session = NewSession {
            session_id: session_id.clone(),
            display_name: "Test Script Mode Session (Windows)".to_string(),
            language: "python".to_string(),
            username: "testuser".to_string(),
            input_prompt: "In [{}]: ".to_string(),
            continuation_prompt: "   ...: ".to_string(),
        ...
        println!("Creating session with StartupEnvironment::Script on Windows");

        // Create the session
        let _created_session_id = create_session_with_client(&client, new_session).await;

        // Start the session
        println!("Starting kernel session with script mode on Windows...");
        let start_response = client
            .start_session(session_id.clone())
            .await
            .expect("Failed to start session");
...
"#;
        ...
        // Verify the environment variable was set correctly
        assert!(
            all_output.contains("TEST_KALLI_VAR=from_script_windows"),
            "Expected to find 'TEST_KALLI_VAR=from_script_windows' in output, got: {}",
            all_output
        );

        println!("✓ Script mode test passed! Environment variable was set correctly on Windows");
    ...
    match test_result {
        Ok(_) => {
            println!("Startup environment script mode test (Windows) completed successfully");
        }
        Err(_) => {
            panic!("Startup environment script mode test (Windows) timed out after 60 seconds");
        }
    }
}
