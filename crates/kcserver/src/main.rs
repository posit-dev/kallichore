...
async fn main() {
    ...
    // Create TcpListener to bind to the port, or Unix domain socket if specified
    #[cfg(unix)]
...
"#,
    ...
    if let Some(connection_file_path) = &args.connection_file {
        match &listener {
            ListenerType::Tcp(_) => {
                if let Err(e) =
                    write_server_connection_file(connection_file_path, port.unwrap(), &token, &args.log_file)
                {
    ...
    let server_listener = match listener {
        ...
        ListenerType::Unix(unix_listener) => {
            server::ServerListener::Unix(unix_listener)
        }
    };

    // Pass the listener to the server
    server::create_with_listener(server_listener, token, args.idle_shutdown_hours, args.log_level).await;
}
